...
ll dp(int i, int k, vector<vector<ll>> &memo, vector<ll> &prefixSum, vector<ll> &B, vector<vector<ll>> &memo2){
	int n = SIZE(prefixSum) - 1; 
	int m = SIZE(B) - 1;
	
	if (i > n && k <= m){ // Si terminé de hacer a A vacío y el k usado es <= m, entonces hice una secuencia válida
		return 0;
	}
...
void solve(vector<ll> &A, vector<ll> &B){
	...
	vector<vector<ll>> memo2(SIZE(A), vector<ll>(SIZE(B), 0));
	...
	ll res = dp(1, 1, memo, prefixSum, B, memo2);
	if (res >= INF){
		cout << -1 << "\n";
	} else {
		cout << memo[1][1] << " " << memo2[1][1] << "\n";
	}
}
