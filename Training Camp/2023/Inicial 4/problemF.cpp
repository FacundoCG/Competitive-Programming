...
int main() {
    ...
    vector<ll> teams(n, 0);
    bool checkBipartite = isBipartite(0, adjList, teams);
    vector<ll> verticesTeam1;
    ...
    ll res = 0;
    ll m = verticesTeam1.size();

    for (auto v : verticesTeam1){
        // Total edges that I can add is: (n-1) - #edges added already - (#sizeTeam1 - 1)
        ll totalEdges = (n-1) - adjList[v].size() - (m-1);
        res += totalEdges;
    }
