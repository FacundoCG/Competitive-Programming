...
struct SuccesorGraph{
	...
	SuccesorGraph(vi &succ, vi &costos) : succesors(succ), costs(costos), BJ(succ, SIZE(succ)) {
	...
	void solve(){
		...
		for (int v : representantsOfCycles) res += minCostCycle(v);
		cout << res << "\n";
	}
};
